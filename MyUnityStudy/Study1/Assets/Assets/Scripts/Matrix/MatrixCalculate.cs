...
public class MatrixCalculate : MonoBehaviour
{
    ...
    private enum CalType
    {
        ...
        TestCreateMatirx = 11,//测试构件矩阵
    }
    private CalType _curType = CalType.ABmulti;
    private const int BUTTON_COUNT = 11;  //按钮个数
    ...
    private void CalTypeSwitch()
    {
        switch (_curType)
        {
            ...
            //case CalType.TestCreateMatirx:
            //    {
                    
            //        float[,] data = new float[,] { { 1, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 } };
            //        Matrix mat = new Matrix(data);
            //        UpdateLabRes(mat.ToString());
            //    } break;
            default:
                {
    ...
    private void UpdateChooseType(string str)
    {
        switch (str)
        {
            ...
            case "测试构建矩阵":
                {
                    _curType = CalType.TestCreateMatirx;
                } break;
        }
